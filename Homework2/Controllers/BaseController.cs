...
namespace Homework2.Controllers
{
        ...
        where TDTO : class
    {
        ...
        public BaseController(IMapper mapper, 
                              UnitOfWork work,
                              GenericRepository<TEntities> repository)
        {
            this._mapper = mapper;
            this._work = work;
        ...
        {
            var varieble = await _repository.GetAsync(id);//_context.Set<TEntities>().FirstOrDefaultAsync(v => v.Id == id);
        ...
        {
            var resul = await _repository.DeleteAsync(id);//_context.Set<TEntities>().Where(e => e.Id == id).ExecuteDeleteAsync();
